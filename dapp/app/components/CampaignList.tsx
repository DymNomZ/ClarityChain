...
const CampaignList: React.FC<CampaignListProps> = ({ account }) => {
  ...
  const handleDonate = async (campaignId: number) => {
    ...
    try {
      ...
      setStatus({ ...status, [campaignId]: { type: "success", message: `Donated ${amount} PAS successfully! Tx: ${hash.slice(0, 18)}...` } });
  ...
  return (
    <div className="space-y-6">
      ...
      {campaigns.map((campaign) => (
        <div
          key={campaign.id}
          className={`rounded-xl border p-5 space-y-4 ${campaign.active ? "border-pink-500 bg-gray-900" : "border-gray-600 bg-gray-800 opacity-60"}`}
        >
