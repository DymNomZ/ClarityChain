...
const CampaignList: React.FC<CampaignListProps> = ({ account }) => {
  ...
  const handleDonate = async (campaignId: number) => {
    ...
    try {
      ...
      setStatus({ ...status, [campaignId]: { type: "error", message: err.message || "Donation failed." } });
    } finally {
  ...
  if (loading) {
    return <div className="text-center text-gray-400 py-12">Loading campaigns...</div>;
  }

  if (campaigns.length === 0) {
    return (
      <div className="text-center text-gray-400 py-12">
        No campaigns yet. NGOs can create one in the NGO Dashboard tab.
      </div>
    );
  }
            ...
            <div className="space-y-2">
              {status[campaign.id]?.message && (
                <div
                  className={`text-sm p-2 rounded-md ${
                    status[campaign.id].type === "error"
                      ? "bg-red-900 text-red-300"
                      : status[campaign.id].type === "success"
                      ? "bg-green-900 text-green-300"
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onChange={(e) =>
                    setDonationAmounts({ ...donationAmounts, [campaign.id]: e.target.value })
                  }
              ...
              {!account && (
                <p className="text-xs text-gray-500">Connect your wallet to donate.</p>
              )}
            </div>
          )}
        </div>
